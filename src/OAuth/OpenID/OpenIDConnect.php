...
namespace Asg\OAuth\OpenID;


use Asg\Exceptions\OpenIDConnectClientException;
use Asg\Http\Client\HttpClientInterface;
use Asg\Storage\StorageInterface;
use Asg\Support\Encoding\Base64;
use Asg\Support\Server\Url;
use Asg\Support\String\String;
...
class OpenIDConnect
{
    ...
    /**
     * @param HttpClientInterface $httpClient; Use to create http request.
     * @param StorageInterface $storage; Use to store session necessary values.
    ...
    public function __construct(HttpClientInterface $httpClient,StorageInterface $storage,
        $provider_url = null, $client_id = null, $client_secret = null
    )
    {
    ...
    private function verifyRSAJWTsignature($hashtype, $key, $payload, $signature)
    {
        ...
        */
        $public_key_xml = "<RSAKeyValue>\r\n" .
            "  <Modulus>" . Base64::base64urlToBase64($key->n) . "</Modulus>\r\n" .
            "  <Exponent>" . Base64::base64urlToBase64($key->e) . "</Exponent>\r\n" .
            "</RSAKeyValue>";
    ...
    {
        $parts = explode(".", $jwt);
        $signature = Base64::base64urlDecode(array_pop($parts));
        $header = json_decode(Base64::base64urlDecode($parts[0]));
    ...
    private function decodeJWT($jwt, $section = 0)
    {
        ...
        return json_decode(Base64::base64urlDecode($parts[$section]));
    }
    ...
    {
        if ($post_body != null) {
            $output = $this->httpClient->post($url,['headers' => $headers,'form_params' => $post_body]);
        }else{
            $output = $this->httpClient->get($url,['headers' => $headers]);
        }
    ...
    protected function setNonce($nonce)
    {
        ...
        $this->storage->put('openid_connect_nonce',$nonce);
        return $nonce;
    }
    ...
    protected function getNonce()
    {
        ...
        return $this->storage->get('openid_connect_nonce');
    }
    ...
    protected function unsetNonce()
    {
        ...
        $this->storage->delete('openid_connect_nonce');
    }
    ...
    protected function setState($state)
    {
        ...
        $this->storage->put('openid_connect_state',$state);
        return $state;
    }
    ...
    protected function getState()
    {
        ...
        return $this->storage->get('openid_connect_state');
    }
    ...
    protected function unsetState()
    {
        ...
        $this->storage->delete('openid_connect_state');
    }
    ...
    private static function hashEquals($str1, $str2)
    {
        ...
        return ($status === 0);*/
        return String::hashEquals($str1,$str2);
    }
}